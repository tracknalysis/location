...
import net.tracknalysis.location.nmea.NmeaRouteManager.RouteState.WaypointState;
import net.tracknalysis.location.nmea.RmcSentence.ModeIndicator;

/**
 * @author David Valeri
 */
class NmeaRouteManager implements RouteManager, NmeaSentenceListener {
    
    private static final Logger LOG = LoggerFactory.getLogger(NmeaRouteManager.class);
    ...
    public void receiveSentence(AbstractNmeaSentence sentence) {
        ...
        for (Map.Entry<Route, RouteState> entry : routeStateMap.entrySet()) {
            ...
            LOG.trace("Calculating info for route {}.  Current state is {}", entry.getKey().getId(), state);
            
            float triggerDistance = state.getTriggerDistance();
            Waypoint waypoint = entry.getKey().getWaypoints().get(state.getNextWaypointIndex());
            ...
            if (state.getWaypointState() == WaypointState.WAITING_TO_REACH) {
                if (currentDistance[0] < triggerDistance) {
                    ...
                    notifyListeners(state.getListeners().keySet(),
                            entry.getKey(), waypoint, gpsTime, systemTime,
                            WaypointEventType.ENTERING_PERIMETER, currentDistance[0]);
                } else {
                ...
                if (currentDistance[0] > triggerDistance) {
                    ...
                    notifyListeners(state.getListeners().keySet(),
                            entry.getKey(), waypoint, gpsTime, systemTime,
                            WaypointEventType.LEAVING_PERIMETER, currentDistance[0]);
                    
                    notifyListeners(state.getListeners().keySet(),
                            entry.getKey(), waypoint,
                    ...
                    // Update the closest distance while we are in the perimeter.
                    if (currentDistance[0] < state.getClosestDistanceToWaypoint()) {
                        if (LOG.isTraceEnabled()) {
                            LOG.trace(
                    ...
                    if (currentDistance[0] < state.getLastDistanceToWaypoint()) {
                        ...
                        notifyListeners(state.getListeners().keySet(),
                                entry.getKey(), waypoint, gpsTime, systemTime, WaypointEventType.APPROACHING,
                                currentDistance[0]);
                    } else if (currentDistance[0] > state.getLastDistanceToWaypoint()) {
                        ...
                        notifyListeners(state.getListeners().keySet(),
                                entry.getKey(), waypoint, gpsTime, systemTime, WaypointEventType.RECEDING,
                                currentDistance[0]);
                    }
                }
            }
    ...
    @Override
    public void addRoute(Route route, float distance, RouteListener... listeners) {
    ...
    protected void notifyListeners(Set<RouteListener> listeners, Route route,
            Waypoint waypoint, long gpsTime, long systemTime,
            WaypointEventType waypointEventType, float distanceToWaypoint) {
        
        for (RouteListener listener : listeners) {
            try {
                listener.waypointEvent(waypoint, route, gpsTime, systemTime, waypointEventType, distanceToWaypoint);
            } catch (Exception e) {
                LOG.error("Error while notifying route listener, " + listener + ".", e);
            }
        }
    }
    ...
    protected static class RouteState {
        ...
        public static enum WaypointState {
            WAITING_TO_REACH,
            REACHED;
        }
